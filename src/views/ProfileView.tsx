...
import { User, ChevronRight, FileText, ClipboardList, Clock, Info, MessageSquareWarning, Settings, Camera } from 'lucide-react';
    ...
    return (
      <div className="flex flex-col h-full bg-gray-50">
        <div className="bg-white px-5 pt-12 pb-4 border-b border-gray-100 flex items-center gap-2 sticky top-0 z-10">
          <button onClick={() => setShowEdit(false)} aria-label="返回" className="text-gray-600 text-sm font-medium">返回</button>
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 mt-2 relative z-10 pb-20 no-scrollbar">
        ...
        <div className="bg-white rounded-xl shadow-custom overflow-hidden">
          <MenuLink icon={FileText} label="维修协议" onClick={onNavigateToAgreement} />
          <MenuLink icon={ClipboardList} label="我的申请" onClick={onNavigateToMyApplications} />
        ...
        <div className="bg-white rounded-xl shadow-custom overflow-hidden">
          ...
          <MenuLink icon={MessageSquareWarning} label="意见反馈" onClick={onNavigateToFeedback} />
          <MenuLink icon={Info} label="关于我们" onClick={onNavigateToAbout} />
        </div>
